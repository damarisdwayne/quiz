...
const Answer = (props: AnswerProps) => {
  ...
  const answerShowed = answer.isShowed ? "transform rotate-y-180" : "";
  return (
    <div
      onClick={() => props.onResponse(props.index)}
      className={`flex h-[100px] m-3 w-4/5 min-w-[500px] ${answerShowed} perspective-1000`}
    >
      <div className="relative flex-1 transform transition-transform duration-800 transform-style-preserve-3d">
        {!answer.isShowed ? (
          <div className="frente absolute flex items-center rounded-xl p-4 gap-5 h-full w-full bg-white backface-hidden">
            <div
          ...
          <div className="absolute flex h-full w-full backface-hidden transform rotate-y-180">
            {answer.isCorrect ? (
